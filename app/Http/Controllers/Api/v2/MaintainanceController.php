...
class MaintainanceController extends Controller
{
    ...
    public function my_maintainance(REQUEST $request)
    {
        $status = (string) 1;
        $message = "";
        $o_data = [];
        ...
        if ($validator->fails()) {
            ...
            $maintainances = Maintainance::orderBy('id', 'desc')
           ...
           if($maintainances){
            ...
            $o_data = convert_all_elements_to_string($maintainances) ;
        }
        return response()->json(['status' => $status, 'errors' => $errors, 'message' => $message, 'oData' => $o_data], 200);
    }
    public function my_maintainance_details(REQUEST $request)
    {
        $status = (string) 1;
        $message = "";
        $o_data = [];
        ...
        if ($validator->fails()) {
            ...
            $user_id = $this->validateAccesToken($request->access_token);
            $maintainance = Maintainance::where(['id' =>  $request->maintainance_id,'user_id' => $user_id])->with('building_list')->first();
           if($maintainance){
               $maintainance->status_text  =  quote_status($maintainance->status);
                $message = "Data Fetched Successfully";
                $bu_type = $maintainance->building_list;
            }else{
                $message = "Data Not Found";
            }
            $o_data = convert_all_elements_to_string($maintainance);
            if(!empty($bu_type)){
    ...
    public function maintainance(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $d = Maintainance::orderBy('id','desc')->get()->first();
            $post_id = $d->id;
            exec("php ".base_path()."/artisan maintanence_push:self ".$post_id." > /dev/null 2>&1 & ");
            exec("php ".base_path()."/artisan maintanence_mail:self ".$post_id." > /dev/null 2>&1 & ");
            //\Artisan::call('maintanence_mail:self '.$post_id);
            $status = "1";
    ...
    function maintenance_change_status(Request $request)
    {
        $status  = "0";
        $message = "";
        $title   = "";
        ...
        if ($validator->fails()) {
            ...
            if ( ! $users ) {
                ...
                if(Maintainance::where('id',$request->maintenance_id)
                    ->update(['status'=>$request->status_id])){
                    $status = "1";
                    
                    if($request->status_id == 1)
                    {
                        $message = "Maintenance Quote Accepted Successfully";
                        $title = 'Maintenance Quote Accepted';
                        $description = "you have successfully accepted quote";
                    }
                    if($request->status_id == 10)
                    {
                        $message = "Maintenance Quote Cancelled Successfully";
                        $title = 'Maintenance Quote Cancelled';
                    ...
                    if (!empty($users->firebase_user_key)) {
                        $notification_data["Notifications/" . $users->firebase_user_key . "/" . $notification_id] = [
                            ...
                            "notificationType" => $ntype,
                            "createdAt" => gmdate("d-m-Y H:i:s", $notification_id),
                            "orderId" => (string) $request->maintenance_id,
                            "Type" => (string) $type,
                            "url" => "",
                            "imageURL" => '',
                            "read" => "0",
    ...
    function maintenance_payment_init(Request $request)
    {
        $status  = "0";
        $message = "";
        $payment_report = [];
        ...
        if ($validator->fails()) {
            ...
            if ( $user ) {
                if(Maintainance::where('id',$request->maintenance_id)->first()){

                    $contract = Maintainance::where('id',$request->maintenance_id)->first();
                    ...
                    if ( $price <= 1.1 ) {
                        ...
                        if (empty($address)) {
                            ...
                            DB::table('maintainance')->where('id', $request->maintenance_id)
    ...
    function verify_maintenance_payment(Request $request)
    {
        $status  = "0";
        $message = "";
        ...
        if ($validator->fails()) {
            ...
            $users = User::where("user_access_token", $request->access_token)->first();
            if ( $users ) {
                if(Maintainance::where('transaction_id',$request->transaction_id)
                    ->update(['status'=>2])){
                    $status = "1";
                    $message = "Successfully Paid Contract Quote";

                    $ontract = Maintainance::where('transaction_id',$request->transaction_id)
                        ->first();
